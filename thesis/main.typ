...
затраты за счет раннего обнаружения проблем в дизайне аппаратных компонентов.
...
подход к описанию и трансформации цифровых схем. Диаграмма иллюстрирующая
соотношение ключевых диалектов, взятая с официального сайта, представлена на рисунке
@img:circt-dialects.
...
#figure(
  image("res/img/circt-dialects.svg",
  width: 65%),
  caption: ""/* TODO */
) <img:circt-dialects>

Пример использования CIRCT в контексте проекта:
...
способами, в зависимости от требований проекта и используемых технологий. Примеры таких технологий рассматриваются в следующих разделах текущей главы.
...
состояния используется наименьший по размеру тип языка Kotlin допускающий весь
диапозон значений. Соответствие типов можно видеть в таблице @tab:bit_to_type_translation.
...
Можно выделить следующие этапы компиляции, представленные в списке с описанием
...
/* TODO выбрать формат: с точной или без */
...
4. Компиляция симуляционной модели в виде LLVM IR в нативный код динамически
  ...
  *Пример*. Файл содержащий нативный код симуляцонной модели в виде LLVM IR
  ...
  видеть на листинге @lst:llvm-compile-commands.
...
Основные элементы коды можно видеть на листинге в приложении В/*@lst:gui-example*/.
...
переполнения, что можно видеть на рисунке @img:counter-clicker-gui-fix. Таким
образом, сквозное тестирование с применением моделей Circulator позволяет
повышать качество разрабатываемых систем.
...
на примере в разделе @пример_с_Controls.kt/*TODO: нумерация*/.
...
говорилось в разделе @требования_к_проекту /* TODO: нумерация */ про
