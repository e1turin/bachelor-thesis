...
версии 0.1.0 включительно. Проект выполнен в соответствии с принципами
мультиплатформенной разработки на языке Kotlin и представляет собой программную
библиотеку и плагин для системы сборки Gradle. Проект структурирован как композитный 
проект в системе сборки Gradle и содержит следующие модули.

- `circulator-core` --- ядро проекта, включаемое во все подпроекты Circulator. Оно реализуется как программная библиотека, предоставляющая абстракции для описания и использования моделей аппаратуры,
  а так же структуры данных для работы со средствами моделирования (Arcilator для указанной версии).
- `circulator-plugin` --- реализация плагина для системы сборки Gradle. Он принимает конфигурацию из сценария сборки, допольнительно конфигурирует проект и генерирует код на языке Kotlin.
...
рисунке /* TODO */. Классы реализцющие интерфейс `StateProjection` выполняют доступ
к данным, расположенным во внешней помяти по отношению к куче JVM, и к ним
делегируется обращение к свойствам класса модели.
...
соответствующей секции, то есть так, как показано на листинге /* TODO */.
...
Плагин принимает конфигурацию в секции своего расширения так, как это показано на листинге /* TODO */. Конфигурация плагина сознательно вынесена в отдельный файл, потому что она может
выглядеть громоздко в конфигурации сборки проекта, а так же такие решение позволяет
разделить конфигурации моделей и проекта.
...
специализированного типом данных, представленного в проекции состояния.

Для каждого стандартного типа проекции имеется соответствующая функция-конструктор, которые
можно видеть на листинге /* TODO */. Такая функция позволяет указать номер байта в сегменте
памяти, соответствующего началу данных отвечающих данной проектции. Используя эти функции
получается компактно определить интерфейс модели --- геттер и сеттер свойства класса
соответствующие проекции состояния делегируются объекту полученному из функции
примерно так: `var clock by input<Byte>(0)`.

```kotlin
inline fun <reified T> Model.input(offset: Long) = // ...
inline fun <reified T> Model.output(offset: Long) = // ...
inline fun <reified T> Model.register(offset: Long) = // ...
inline fun <reified T> Model.memory(offset: Long) = // ...
inline fun <reified T> Model.wire(offset: Long) = // ...
```
...
Модель прибора создается в контроллере посредством менеджера устройств
...
приложения, реализующего возможность ручного и автоматического увеличения счета
и сброса. Внешний вид этого приложения можно видеть на рисунке /* TODO */.
...
использовать для построения системы и поиска недостатков. Стоит сказать, что
приложение работает достаточно быстро, и задержка обновления счета по нажатию
