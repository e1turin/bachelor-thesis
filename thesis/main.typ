...
/ Мультиплатформенный: (от англ. multiplatform) условное обозначение для
                       программного кода или проекта работать на множестве
...
/ Нативный: (от англ. native --- "родной") условное название для компонента,
            ...
            контексте платформы JVM так же может значить "внешний", то есть
...
/ Сорсет: (от англ. source set) короткое условное обозначение для набора
          исходных файлов, относящихся к конкретной платформе; используется в
          проектах Kotlin Multiplatform, например, common сорсет --- набор
          исходных файлов общий для всех платформ, native сорсет --- специфичный
          для Native платформы
...
/ CIRCT: название проекта и набора технологий для работы с моделями цифровой
         техноки, использующих технологии компиляторной инфраструктуры LLVM
...
/ JDK: пакет инструментов разработки приложений на языке Java, специализируется
       версией, например, JDK 22 обозначает 22 версию языка, в которой стало доступно новое FFM API
...
устрайствами, что расширяет границы применимости фреймворка. @controls-next-gen @controls-novel-solution Кроме того
...
известном, как
#quote([''Troisk nu-mass,, experiment]) @numass.
...
инструментами. В ранних версиях язык полагался на самостоятельно разработанное
промежуточное представление FIRRTL (Flexible Intermediate Representation for
RTL), но в новых версиях проект, развиваший FIRRTL, объявлен архивным и
разработчики рекомендуют использовать промежуточное представление из проекта
LLVM CIRCT. Изначально инструмент был разработан в Калифорнийском университете в
Беркли, а теперь развивается организацией Chips Alliance The Linux Foundation.
/* TODO ссылка на офф сайт */ Этот язык широко применяется для генерации
...
сам компилируется в этот язык. @verik
...
представлена на рисунке @img:circt-technologies. @circt-presentation
...
выглядит трудоемким. @systemc-book /* TODO ссылки на доку */
...
принципам мультиплатформенной разработки и есть потенциал реализовать проект с
поддержкой платформ отличных от JVM, например, Native или WASM.
