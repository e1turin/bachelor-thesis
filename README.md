...
Остановился на названии "Circulator.kt" для библиотеки на Kotlin - https://e1turin/circulator-kt.
...
Моделирование происходит в режиме "cycle-accurate", что переводится как "тактовое моделирование" или "моделированию с тактовой точностью". 
В [статье][https://habr.com/ru/articles/884396/] это называется просто "цифровое моделирование при помощи Verilog".


Мне нравится 3 вариант цели, где мы все таки останавливаемся на E2E тестировании
с учетом цифровых схем. Можно задаться вопросом, а так ли оно нужно. И для иллюстрации стоит привести какой-то пример системы, где ошибка проявляется из-за сложного взаимодействия двух приборов. 
- Например, можно вспомнить о недокументированных возможностях в bluetooth
  чипах, которые реагировали на сообщения, содержащие не явные команды. Так вот, положим, что у нас есть в системе такое устройство, которое неожиданно флипает бит на последовательность сообщений `0b11111111` (которое не должно появляться в эфире), а другое устройство в режиме ожидания как раз производит именно такой сигнал.
...
- Еще пример, мы можем тестировать устройство, которое предполагает защиту от помех. А помехи 
  могут происходить только от определенного устройства в определенном его состоянии. И вот мы проверим их интеграцию.
...
1. Документация и исходные коды проекта Controls.kt.
2. Исходные коды и документация проектов реализованным с
   использованием фреймворка Controls.kt.
3. Документация и исходные коды проекта LLVM CIRCT и связанных с ним репозиториев.
4. Документация проектов Chisel и Verilator.
...
Повышение качества систем, использующих фреймворк "Controls.kt" для реализации, за
счет их сквозного тестирования c применением моделей цифровых схем в виртуальных устройствах.
...
- Оценка затрат: выбрасываем промежуточные модели, сразу тестируем на актуальных
  ...
  модели.
  генерации моделей мы добиваемся преимущества.
...
- Разработка программного модуля на языке программирования Kotlin для построения моделей аппаратуры.
...
2. Описание и документация FFM API в Java из проекта Panama - https://openjdk.org/projects/panama/ (дата обращения ...)
3. Документация проекта LLVM CIRCT - <!-- ссылка --> (дата ...)
4. 
